...
window.VencordNative = {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Coffecord/Coffecord" }),
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("CoffecordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("CoffecordSettings", JSON.stringify(s)),
